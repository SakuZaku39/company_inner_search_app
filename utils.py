...
def detect_employee_query(query: str) -> bool:
    ...
    employee_action_patterns = [
        ...
        "人事部.*従業員", "営業部.*従業員", "開発部.*従業員", "経理部.*従業員", "マーケティング部.*従業員", "総務部.*従業員",
        "人事部.*社員", "営業部.*社員", "開発部.*社員", "経理部.*社員", "マーケティング部.*社員", "総務部.*社員",
        "人事部.*スタッフ", "営業部.*スタッフ", "開発部.*スタッフ", "経理部.*スタッフ", "マーケティング部.*スタッフ", "総務部.*スタッフ",
...
def simple_employee_search(query: str) -> dict:
    ...
    try:
        ...
        # クエリから部署を抽出
        departments = ["人事部", "営業部", "開発部", "経理部", "マーケティング部", "総務部", "IT部"]
        found_dept = None
