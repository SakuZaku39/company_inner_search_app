...
SUPPORTED_EXTENSIONS = {
    ...
    ".txt": lambda path: TextLoader(path, encoding="utf-8")  # 修正点: .txtファイルサポート追加
}
...
# RAG処理設定系（マジックナンバー対策）
# ==========================================
# ベクターストアから取得する関連ドキュメント数（問題1で3→5に変更）
RAG_SEARCH_K = 5

# ドキュメントのチャンク分割設定（修正点: RAG検索精度向上のため調整）
CHUNK_SIZE = 800           # チャンクの最大文字数（500→800に増加：より多くの文脈を保持）
CHUNK_OVERLAP = 100        # チャンク間の重複文字数（50→100に増加：文脈の連続性向上）
CHUNK_SEPARATOR = "\n"     # チャンク分割の区切り文字
...
# ==========================================
# Streamlitのカラム比率設定
UI_COLUMN_RATIO_MAIN = 100    # メインカラムの比率
UI_COLUMN_RATIO_SPACER = 1    # スペーサーカラムの比率

# コンテキストドキュメントのインデックス
MAIN_DOCUMENT_INDEX = 0       # 最も関連性が高いメインドキュメントのインデックス
SUB_DOCUMENTS_START_INDEX = 1 # サブドキュメントの開始インデックス（スライス用）
...
SYSTEM_PROMPT_DOC_SEARCH = """
    ...
    【条件】
    1. ユーザー入力内容と以下の文脈との間に関連性がある場合（部分的でも関連があれば）、空文字「""」を返してください。
    2. 検索キーワードが直接含まれていなくても、意味的に関連する内容があれば関連ありと判定してください。
    3. 例：「社員の育成」→「教育」「研修」「スキルアップ」「人材開発」なども関連として扱う
    4. 例：「MTGの議事録」→「会議」「ミーティング」「打合せ」なども関連として扱う
    5. 明らかに全く関係のない内容の場合のみ、「該当資料なし」と回答してください。
